...
function getAlias (selector, log, cy) {
  ...
  return () => {
    ...
    try {
      ...
      // @ts-expect-error - vendored: getAliasedRequests signature
      const requests = getAliasedRequests(strippedAlias, cy.state)

      // @ts-expect-error - vendored: isDynamicAliasingPossible signature
...
export default (Commands, Cypress, cy, state) => {
  ...
  Commands.addQuery('shadow', function contains (userOptions: Cypress.LogTimeoutOptions) {
    // @ts-expect-error - vendored: LogTimeoutOptions union
